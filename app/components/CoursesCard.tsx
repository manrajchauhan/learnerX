...
interface ProductCardProps {
  ...
  const CoursesCard: React.FC<ProductCardProps> = ({ imageSrc, name, currentPrice, originalPrice,link }) => {
    ...
    <Link href={link} className="flex flex-col flex-1 shrink bg-white border basis-0 border-neutral-400 min-w-[240px]">
      <img loading="lazy" src={imageSrc} alt={`Product: ${name}`} className="object-cover w-full rounded-md" />
      <div className="flex flex-col p-4 w-full">
        <h2 className="text-2xl leading-snug tracking-wide">{name}</h2>
        <img loading="lazy" src="/under.svg" alt="" className="object-cover mt-4 w-full aspect-[142.86] stroke-neutral-900" />
